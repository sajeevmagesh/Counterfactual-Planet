...
def prepare_data():
    ...
    country_year_past_policies = defaultdict(list)
    country_year_current_policies = defaultdict(list)
    country_year_co2 = defaultdict(float)
    for _, row in df.iterrows():
        country_time_range = row["country time range"]
        country = country_time_range.split("_")[0]
        year = country_time_range.split("_")[1]
        past_year = country_time_range.split("_")[2]

        if int(year) - int(past_year) > 0:
            country_year_past_policies[country + "_" + year].append((past_year, row['policies']))
        else:
            country_year_current_policies[country + "_" + year] = row['policies']

        if int(year) - int(past_year) == 1 and int(year) >= 2000:
            country_year_co2[country + "_" + year] = row["delta co2"]

    past_policies = pd.DataFrame(list(country_year_past_policies.items()), columns=['country time', 'past policies'])
    current_policies = pd.DataFrame(list(country_year_current_policies.items()), columns=['country time', 'current policies'])
    co2 = pd.DataFrame(list(country_year_co2.items()), columns=['country time', 'delta co2'])

    policies = pd.merge(past_policies, current_policies, on='country time', how='outer')
    df = pd.merge(policies, co2, on='country time', how='outer')
    df = df.dropna()
...
def generate_prompts(df):
    ...
    for _, row in df.iterrows():
        ...
        if type(row['past policies']) != list and pd.isna(row['past policies']):
            prompt += "No policies were enacted.\n"
        else:
            row['past policies'] = sorted(row['past policies'])
            for past_year, policies in row['past policies']: 
                nonzero = set()
                for stringency, policy in policies:
                    if stringency > 0 or policy in nonzero:
                        prompt += f"{country} implemented a {policy} policy with stringency {stringency} in {int(past_year)}\n"
                        nonzero.add(policy)
        ...
        for stringency, policy in row['current policies']:
